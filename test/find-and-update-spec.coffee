    ...
    describe 'when the device is updated', ->
      beforeEach (done) ->
        update =
          $set:
            foo: 'bar'
        @sut.findAndUpdate {uuid:'wet-sock', updatedBy:'foo', data:update}, (error, @data) => done error

      it 'should return the unmodified record', ->
        expect(@data).to.deep.equal uuid: 'wet-sock'
    ...
    describe 'when the device is updated with a $ key at the top', ->
      beforeEach (done) ->
        update =
          $set:
            $ref: 'bar'
        @sut.findAndUpdate {uuid:'wet-sock',data:update}, (error) => done error
