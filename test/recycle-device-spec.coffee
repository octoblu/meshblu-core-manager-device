...
describe 'Recycle Device', ->
  ...
  describe 'when the device exists', ->
    ...
    beforeEach (done) ->
      @sut.recycle {uuid:'should-be-removed-uuid'}, (error) => done error

    it 'should copy the device to the deleted collection', (done) ->
      @datastore.findOneRecycled {uuid: 'should-be-removed-uuid'}, (error, device) =>
  ...
  describe 'when the device doesn\'t exists', ->
    beforeEach (done) ->
      @sut.recycle {uuid:'unknown-uuid'}, (@error) => done()
  ...
  describe 'when called without a uuid', ->
    beforeEach (done) ->
      @sut.recycle {uuid: null}, (@error) => done()
